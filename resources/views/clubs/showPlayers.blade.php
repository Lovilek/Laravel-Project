...
<div class="w3-w3-card-4 w3-center  " style="width:75% ;padding-left:310px;" >
    ...
    <div class="w3-card-4 w3-margin w3-white" >
        @foreach($Clubs as $Players)
            @if($Players->football_clubs_id==$id_club)
                <img src="@foreach($pictures as $pic) @if($pic->id==$Players->image_id)  /storage/{{$pic->imgSrc}}    @endif @endforeach" alt="Nature" style="width:100%;" />
        <div class="w3-container">
            <h3><b> {{$Players->name}} {{$Players->surname}} </b></h3>
        </div>
        <div class="w3-container">
            <p>{{$Players->position}} Возвраст {{$Players->age}}  Стоимость {{$Players->price}}$</p>
